...
DESCRIPTION
-----------
*exjld* binary consume view files defined in format specified of *viewfile(5)*.
...
To perform offset and size calculation, exjld generates small C program which is
...
by exjld. In that case exjld will ignore the meta data and will produce new
...
*exjld* must have access to *cc* (C Compiler).
